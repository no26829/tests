# test different LLMs: Claude
# Embedding pair: UniversalSentenceEncoder

from langchain import SonnetClaude, LLMChain
from langchain.embeddings import UniversalSentenceEncoderEmbeddings
import os
import logging
...
# Claude API Key placeholder ('CLAUDE_API_KEY')
api_key = os.getenv('CLAUDE_API_KEY')
if not api_key:
    raise ValueError("API key not found. Please set the CLAUDE_API_KEY environment variable.")

# Initialize the Universal Sentence Encoder embeddings
embedding_model = UniversalSentenceEncoderEmbeddings()

# Setup the LLMChain with the embedding model
llm_chain = LLMChain(llm=SonnetClaude(api_key=api_key), embedding_model=embedding_model)

def get_menu_from_restaurant_claude(name, address):
    try:
...
def workflow(name, address):
    menu = get_menu_from_restaurant_claude(name, address)
