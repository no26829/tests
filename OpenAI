...
# OpenAI API Key placeholder ('OPENAI_API_KEY')
#(got a free key but said there was restrictions at my account level)
...
llm_chain = LLMChain(llm=OpenAI(api_key=api_key), embedding_model=embedding_model)

def get_menu_from_restaurant_openai(name, address):
    try:
...
def workflow(name, address):
    menu = get_menu_from_restaurant_openai(name, address)
